...
USceneCapturer::USceneCapturer(FVTableHelper& Helper)
	...
	, CombineAtlasesOnOutput(FStereoPanoramaManager::CombineAtlasesOnOutput->GetInt() != 0)
	, stereo(FStereoPanoramaManager::stereo->GetInt() != 0)
	//, OutPutFileExtension(FStereoPanoramaManager::OutPutFileExtension->GetString())
{}
...
{
	//UE_LOG(LogStereoPanorama, Log, TEXT("Tipo do folder..."));
	//UE_LOG(LogStereoPanorama, Log, TEXT("%s"), *Folder);
	...
	//If from VRMonkey
	if (CombineAtlasesOnOutput && !stereo) {
		UE_LOG(LogStereoPanorama, Warning, TEXT("If SP.CombineAtlasesOnOutput is '1', SP.Stereo should be '1' as well"));
	}

	//IF from VRMonkey
	if (Folder.Equals("Right") || stereo) {
		// Generate name
		FString FrameString = FString::Printf(TEXT("%s_%05d.jpg"), *Folder, CurrentFrameCount);
		FString AtlasName = OutputDir / Timestamp / FrameString;

		UE_LOG(LogStereoPanorama, Log, TEXT("Writing atlas: %s"), *AtlasName);

		// Write out PNG
		//VRMonkey change to JPEG
		//TODO: ikrimae: Use threads to write out the images for performance
		IImageWrapperPtr ImageWrapper = ImageWrapperModule.CreateImageWrapper(EImageFormat::JPEG);

		//VRMonkey begin
		//made to create the both image(right and left) in the same image file
		...
		{
			FString FrameStringUnprojected = FString::Printf(TEXT("%s_%05d_Unprojected.jpg"), *Folder, CurrentFrameCount);
...
void USceneCapturer::Tick(float DeltaTime)
{
	...
	else
	{
		...
		TArray<FColor> SphericalRightEyeAtlas;
		//VRMonkey begin
		...
		else if (!CombineAtlasesOnOutput && stereo) {
			SphericalRightEyeAtlas = SaveAtlas(TEXT("Right"), UnprojectedRightEyeAtlas);
		}
